...
class TikTok():
    ...
    """
        ...
        # 用户主页      # 保存路径      # 单页下载数        # 下载音频      # 下载模式      # 保存用户名            # 点赞个数            # 用户唯一标识
        self.uid = '';self.save = '';self.count = '';self.musicarg = '';self.mode = '';self.nickname = '';self.like_counts = 0;self.sec = ''
        ...
        if os.path.isfile("conf.ini") == True:
            ...
            try:
                ...
                self.cf.set("url", "uid", "https://v.douyin.com/8PUx1hQ/")
    ...
    #         print('----为您下载多个视频!----\r')
    #         # 获取用户sec_uid
    #         key  = r.url[28:107]  # key  = r.url[28:83] 旧
    #         print('[  提示  ]:用户的sec_id=%s!\r' % key)
    #         print('3' + r.url + '\n4' + key)
    ...
    def videos_download(self, author_list, video_list, uri_list, aweme_id, nickname, max_cursor):
        ...
        for i in range(len(author_list)):
            ...
            try:
                if self.musicarg == "yes":                              # 保留音频
                    ...
                    if music.status_code == 200:                        # 判断是否响应成功
                        ...
                        if self.mode == 'post':
                            m_url = self.save + "\\" + nickname[i] + '\\' + creat_time + re.sub(r'[\\/:*?"<>|\r\n]+', "_", music_title) + '_' + author_list[i] + '.mp3'
                        else:
                            m_url = self.save + "\\" + self.nickname + '\\' + creat_time + re.sub(r'[\\/:*?"<>|\r\n]+', "_", music_title) + '_' + author_list[i] + '.mp3'
            ...
            try:
                ...
                t_video = requests.get(url=new_video_list[i], headers=self.headers).content                       # 视频内容
                ...
                try:
                    if video.status_code == 200:                        # 判断是否响应成功
                        ...
                        if self.mode == 'post':
                            v_url = self.save + "\\" + nickname[i] + '\\' + creat_time + re.sub(r'[\\/:*?"<>|\r\n] + ', "_", author_list[i]) + '.mp4'
                        else:
                            v_url = self.save + "\\" + self.nickname + '\\' + creat_time + re.sub(r'[\\/:*?"<>|\r\n] + ', "_", author_list[i]) + '.mp4'
